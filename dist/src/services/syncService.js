...
function classifyOtherWorks(remarks) {
...
async function insertEvent(client, hawkerCentreId, eventType, title, startDate, endDate, remarks, sourceQuarter) {
...
export async function syncFromNea() {
      ...
      `);
        ...
        try {
            ...
            for (const record of records) {
                ...
                const otherWorksInserted = await insertEvent(client, hawkerCentreId, classifyOtherWorks(otherWorksRemarks), 'Other works', parseSingaporeDate(getField(record, 'other_works_startdate')), parseSingaporeDate(getField(record, 'other_works_enddate')), otherWorksRemarks, 'OTHER_WORKS');
                if (otherWorksInserted)
                    eventsInserted += 1;
            }
            await client.query(`UPDATE sync_runs SET status = 'success', rows_processed = $2, notes = $3, completed_at = NOW() WHERE id = $1`, [syncRunId, records.length, `Inserted ${eventsInserted} closure events`]);
            return { rowsProcessed: records.length, eventsInserted };
        }
        catch (error) {
            await client.query(`UPDATE sync_runs SET status = 'failed', rows_processed = $2, notes = $3, completed_at = NOW() WHERE id = $1`, [syncRunId, records.length, error instanceof Error ? error.message : 'Unknown error']);
            throw error;
        }
    });
}
